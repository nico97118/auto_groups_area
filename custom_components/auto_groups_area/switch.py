"""Switch platform for Auto Groups by Area.

Creates one switch entity per area, by forwarding turn_on/turn_off to member
switches and exposing aggregated state.
"""
...
async def async_setup_entry(
    ...
    """Set up the Auto Groups by Area switch platform."""
    coordinator = AreaSwitchGroupCoordinator(hass, config_entry, async_add_entities)
...
class AreaSwitchGroupCoordinator:
    """Coordinate area switch-group creation and updates."""
    ...
    def __init__(
        ...
        self.groups: dict[str, AreaSwitchGroup] = {}
    ...
    async def _async_update_group_for_area(
        ...
        if not member_entity_ids and not create_when_empty:
            ...
            if existing is not None:
                _LOGGER.info("Removing empty switch group for area '%s'", area.name)
        ...
        _LOGGER.info(
            "Created switch group for area '%s' with %d member(s)",
            area.name,
...
class AreaSwitchGroup(SwitchEntity):
    """A switch entity representing all switches in an area."""
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        """Forward turn_on to member switches."""
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Forward turn_off to member switches."""
        if not self._member_entity_ids:
            return
        await self.hass.services.async_call(
            "switch",
