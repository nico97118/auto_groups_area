...
class AreaSensorGroupCoordinator:
    ...
    async def async_start(self) -> None:
        ...
        _LOGGER.debug(
            "Starting sensor coordinator (entry_id=%s, create_when_empty=%s, include_device_area=%s)",
        ...
        _LOGGER.debug(
            "Deferring initial sensor scan until %s (entry_id=%s)",
    ...
    async def async_update_all_groups(self) -> None:
        ...
        _LOGGER.debug(
            "Sync sensor groups start (entry_id=%s, areas_total=%d, existing_groups=%d)",
            self.config_entry.entry_id,
            len(areas),
            len(self.groups),
        )
        ...
        _LOGGER.debug(
            "Sync sensor groups done (entry_id=%s, areas_total=%d, groups_now=%d)",
            self.config_entry.entry_id,
            len(areas),
            len(self.groups),
        )
    ...
    async def _async_update_groups_for_area(
        ...
        for group_key, (
            ...
            _LOGGER.debug(
                "Area '%s' sensor group '%s' (aggregation=%s, members=%d, ids=%s)",
                area.name,
                group_key,
                aggregation,
            ...
            if not member_entity_ids and not create_when_empty:
                _LOGGER.debug(
                    "Skipping empty sensor group '%s' for area '%s' (create_when_empty=false)",
                    group_key,
                    area.name,
                )
            ...
            if unique_id in self.groups:
                ...
                if member_entity_ids != group._member_entity_ids:
                    ...
                    _LOGGER.debug(
                        "Updating sensor group '%s' for area '%s' (added=%s, removed=%s)",
                        group_key,
                        area.name,
            ...
            _LOGGER.info(
                "Created %s sensor group for area '%s' with %d member(s)",
                group_key,
                area.name,
                len(member_entity_ids),
            )
    ...
    def _handle_entity_registry_updated(self, event: Event) -> None:
        ...
        _LOGGER.debug(
            "Entity registry updated for sensor (entity_id=%s, old_area_id=%s, new_area_id=%s)",
            entity_id,
            old_area_id,
            new_area_id,
        )
    ...
    def _handle_area_registry_updated(self, event: Event) -> None:
        ...
        _LOGGER.debug(
            "Area registry updated for sensor (action=%s, area_id=%s)",
            action,
            area_id,
        )
    ...
    def _handle_device_registry_updated(self, event: Event) -> None:
        ...
        new_area_id: str | None = event.data.get("area_id")
        _LOGGER.debug(
            "Device registry updated for sensor (action=%s, old_area_id=%s, new_area_id=%s)",
            action,
            old_area_id,
            new_area_id,
        )
